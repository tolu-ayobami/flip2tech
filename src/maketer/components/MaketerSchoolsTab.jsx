...
import { PulseLoader } from "react-spinners";
import VolunteerSchoolCard from "../../components/VolunteerSchoolCard";
...
export default function MaketerSchoolsTab() {
  const { inlineLoader, mySchools } = useMaketerContext();
  ...
  return (
    ...
    >
      <Tabs.Item
        title={`My Schools (${mySchools ? mySchools?.length : 0})`}
        className=""
        activeClassName="active-tab"
      >
        {!inlineLoader && (mySchools?.length === 0 || !mySchools) && (
        ...
        {!inlineLoader && mySchools?.length > 0 && (
          <div className="w-full mt-8 flex gap-4 md:gap-6 md:flex-wrap overflow-x-auto md:overflow-x-hidden pb-[80px]">
            {mySchools?.map((item, idx) => {
              return <VolunteerSchoolCard item={item} key={idx} />;
            })}
