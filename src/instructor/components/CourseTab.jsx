...
export default function CourseTab({ setShowOptions, showOptions }) {
  const { inlineLoader, allCourses } = useAdminContext();
  const { userData } = useAppContext();

  // console.log("userData", userData);
  // console.log("allCourses", allCourses);

  const sortedCourses = allCourses?.sort((a, b) => b?.id - a?.id);
  ...
  return (
    ...
    >
      <Tabs.Item
        title={`Published (${allCourses ? allCourses?.length : 0})`}
        className=""
        activeClassName="active-tab"
      >
        {!inlineLoader && (allCourses?.length === 0 || !allCourses) && (
        ...
        {!inlineLoader && allCourses?.length > 0 && (
